...
- Regrading reads, what does the blue and pink color indicate ?
...
- In the **search panel** select **Human**. In the text area of the search panel type **ENSMUSG00000038418**.
...
- Ask for **Show transcript table**. How many transcript are associated with EGR1 ?
...
We will now select differentially expressed genes by applying a threshold on the **adjusted p-value (padj).

<div class="protocol">
- Select **Filter and Sort > Filter data on any column using simple expressions**.
- Set **Number of header lines to skip** to 1.
- Apply the following filter **c7 < 0.01** (i.e adjusted p-value < 0.01).
- How many lines (genes) do you obtain as output ?
- Cut the column 1 (gene name) using **Text Manipulation > cut** and save it in a dataset called **differentially_expressed_genes**.
</div>

Now we will join this list with the matrix (**differentially_expressed_genes**) with the count matrix (DM_vs_PI_gene_counts.txt).

<div class="protocol">
- Use **Join, Subtract and Group > Join two Datasets.** 
- Set **Join** to **differentially_expressed_genes**.
...
- Set **From** to the joined dataset.
- How many lines (genes) do you obtain as output ?
- To retrieve the original header use **Text Manipulation > Select first lines from a dataset** set **Select first** to 1 and press execute.
- Use **Text Manipulation > Concatenate datasets tail-to-head** to concatenate the header and the previous dataset (output of cut).
- Save this dataset as **differentially_expressed_genes_matrix**
</div>


## Hierarchical clustering with cluster.

We will now apply a hierarchical clustering (class discovery) to check wether our selected genes are able to distinguish between PI and DM samples. First we will need to transform data into log2 scale.

<div class="protocol">
- Select **Convert Formats > Add a pseudocount and perform log2 transformation**.
- Choose **differentially_expressed_genes_matrix** as input.
...
Now we can load the result un **Java Treeview**.

<div class="protocol">
- launch **Java Treeview**.
- Use **File > Open** and browse to the .cdt file.
- Go to **Settings > Pixel settings** and set **Global > Y > Fill** and **contrast** to 2.
</div>
...
### Test alternative methods for differential analysis
